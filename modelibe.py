...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        pre_nms_limit = min(6000, self.anchors.shape[0])  # 选择不超过6000个
...
def overlaps_graph(boxes1, boxes2):
    ...
    intersection = tf.maximum(x2 - x1, 0) * tf.maximum(y2 - y1, 0)  # 计算交集
    # 3. Compute unions
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    overlaps = overlaps_graph(proposals, gt_boxes)  # 计算io
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    """
    # Load image and mask
    image = dataset.load_image(image_id)  # load image use skimage
    ...
    bbox = utils.extract_bboxes(mask)  # computer bbox from mask awsome
...
class MaskRcnn(object):
    ...
    def build(self, mode, config):
        ...
        _, C2, C3, C4, C5 = resnet_graph(input_image, "resnet101", stage5=True)  # resnet101的输出
        # Top-down Layers
        ...
        # Generate Anchors
        self.anchors = utils.generate_pyramid_anchors(config.RPN_ANCHOR_SCALES,  # 五个尺度对应 [P2, P3, P4, P5, P6]
                                                      config.RPN_ANCHOR_RATIOS,  # 对应比例
                                                      config.BACKBONE_SHAPES,  # 特征图的大小
                                                      config.BACKBONE_STRIDES,  # 下采样的比例
                                                      config.RPN_ANCHOR_STRIDE)  #
        ...
        rpn_rois = ProposalLayer(proposal_count=proposal_count,  # 提取Proposal的数量
                                 nms_threshold=config.RPN_NMS_THRESHOLD,  # 做nms的阈值
                                 name="ROI",
                                 anchors=self.anchors,
